...
class STM_TGM_Plugins
{
	...
	{
		if ( !wp_doing_ajax() ) add_action( 'admin_init', array( self::class, 'set_theme_plugins' ) );
	...
	private static function set_plugins_addtionals_info()
	{
		...
		foreach ( self::$themePlugins as $k => $plugin ) {
			$plugin['file_path'] = $k . '/' . $plugin['slug'] . '.php';
			$plugin['sort'][] = 'all';
	...
	public static function get_plugins_navigate_view()
	{
		$navItems = array();
		...
		foreach ( self::$plugData as $k => $items ) {
			if ( count( $items ) > 0 ) {
				$navItems[$k] = array(
					'label' => $labels[$k],
					'count' => count( $items )
	...
	public static function install_plugin( $slug, $source, $filePath )
	{
		...
		if ( empty( $source ) ) wp_send_json( array( 'error' => 'Plugin source can not be empty' ) );
		...
		if ( !empty( $installed->get_error_messages() ) ) {
	...
	public static function get_plugin_data_from_api()
	{
		...
		if(is_wp_error($response)) wp_send_json($response->errors);
		
		unset( $response->sections );
		
		if ( !empty( $response->author ) && empty( $plug_data['author'] ) ) $plug_data['author'] = str_replace( '">', '" target="_blank">', $response->author );
		if ( !empty( $response->version ) && empty( $plug_data['version'] ) ) $plug_data['version'] = $response->version;
		if ( !empty( $response->download_link ) && empty( $plug_data['download_link'] ) ) $plug_data['download_link'] = $response->download_link;
