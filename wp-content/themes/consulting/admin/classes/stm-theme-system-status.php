...
Class STM_Theme_System_Status
{
	...
	private static function map_server_env()
	{
		$mapServerEnv = array(
			'server_info' => [
				'title' => 'Server Info',
				'recommend' => '-',
				'system' => isset( $_SERVER['SERVER_SOFTWARE'] ) ? esc_html( sanitize_text_field( wp_unslash( $_SERVER['SERVER_SOFTWARE'] ) ) ) : 'Unknown'
			],
			'mysql_version' => [
				'title' => 'MySQL Version',
			...
			'gzip' => [
				'title' => 'GZip',
				'recommend' => 'enabled',
				'system' => ( substr_count( $_SERVER['HTTP_ACCEPT_ENCODING'], 'gzip' ) > 0 ) ? 'enabled <i class="stmadmin-icon-imported-check"></i>' : 'disabled'
			],
			'ziparchive' => [
				'title' => 'ZipArchive',
				'recommend' => 'enabled',
	...
	private static function get_upgrade_version()
	{
		...
		return $updTrans->updates[0]->current;
	}
	...
	private static function get_php_max_input_vars_system()
	{
		...
		if ( $max_input_vars < $required_input_vars ) {
			self::$notification = true;
			$inputVars = '<span class="plug_bold">' . apply_filters( 'stm_theme_esc_variable', $required_input_vars ) . '</span><i class="stmadmin-icon-round-error"></i>';
		} else {
			$inputVars = $required_input_vars . '<i class="stmadmin-icon-imported-check"></i>';
		}
